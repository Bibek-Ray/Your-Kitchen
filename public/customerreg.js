(function ($) {
    ...
    if (marginSlider != undefined) {
        noUiSlider.create(marginSlider, { start: [500], step: 10, connect: [true, false], tooltips: [true], range: { min: 0, max: 1000 }, format: wNumb({ decimals: 0, thousand: ",", prefix: "$ " }) });
    }
    $("#reset").on("click", function () {
        $("#register-form").reset();
    });
    $("#register-form").validate({
        rules: { first_name: { required: true }, last_name: { required: true }, company: { required: true }, email: { required: true, email: true }, phone_number: { required: true } },
